...
- `hermes-agent` (official installer + local CLI + dashboard user service)
...
- `hermes-agent` は upstream installer を使い、`~/.hermes/hermes-agent` と `~/.local/bin/hermes` を作成する。`hermes-dashboard.service` は `devpc:9119` で dashboard を公開し、既存の `opencode.service` は置き換え対象として停止・無効化する。BasicAuth の dashboard login が `/auth/login` に誤誘導される upstream 版では、同梱 patch をインストール後に自動適用する。model / provider 設定は `hermes setup` で手動設定する
